...
#read txt and converts to data.frame
retrievetables <- function (filename,cols = NULL){
...
getdata <- function(type, features){
  ...
  y_data <- retrievetables(paste(type,"/","y_",type,".txt",sep=""),"activity")
  x_data <- retrievetables(paste(type,"/","X_",type,".txt",sep=""),features$V2)
...
#3) Uses descriptive activity names to name the activities in the data set
activity_labels <- retrievetables("activity_labels.txt")
